...
func getServiceType(a Action) (services.ServiceAction, error) {
	switch a.Todo {
	case ActionDisable:
	...
	default:
		panic(fmt.Sprintf("unexpected materia.ActionType: %#v", a))
	}
}

func modifyService(ctx context.Context, sm ServiceManager, command Action, timeout int) error {
...
func (e *Executor) executeAction(ctx context.Context, v Action) error {
	...
	case components.ResourceTypeVolume:
		...
		case ActionEnsure:
			err := modifyService(ctx, e.host, Action{
				Todo:   ActionReload,
...
func (e *Executor) Execute(ctx context.Context, plan *Plan) (int, error) {
	if plan.Empty() {
		return -1, nil
	}
